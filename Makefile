...
docker-run: docker-rm docker-build
	...
	sudo docker run -it -e nworkers=$(NPROC) -e cpuload=50 -e timeout=20 $(PROJECT)

## limited to equivalent of 1 host cpu, spreads 50% load across all host cpu
docker-run-1cpu-50: 
	sudo docker run -it --cpus=1.0 -e nworkers=$(NPROC) -e cpuload=50 -e timeout=20 $(PROJECT)

## limited to equivalent of 2 host cpu, spreads 50% load across all host cpu
docker-run-2cpu-50: 
	sudo docker run -it --cpus=2.0 -e nworkers=$(NPROC) -e cpuload=50 -e timeout=20 $(PROJECT)

## limited to equivalent of 1/2 host cpu, spreads 100% load across all host cpu
docker-run-halfcpu-100:
	sudo docker run -it --cpus=0.5 -e nworkers=$(NPROC) -e cpuload=100 -e timeout=20 $(PROJECT)

## limited to equivalent of 2 host cpu, spreads 100% load pinned to cpu 0 and 1
docker-run-2cpu-pinned100:
	sudo docker run -it --cpus=2.0 --cpuset-cpus=0,1 -e nworkers=$(NPROC) -e cpuload=100 -e timeout=20 $(PROJECT)
